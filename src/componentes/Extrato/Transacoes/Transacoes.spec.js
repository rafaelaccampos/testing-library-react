...
describe('Transações', () => {
  it('Deve renderizar as transações com as propriedades atualizadas', () => {
    const transacao = {
      transacao: 'Depósito',
      valor: 100,
    };
    ...
    const valorNovaTransacao = screen.getByTestId('valorTransacao');

    expect(novoTipoTransacao).toHaveTextContent('Transferência');
    expect(valorNovaTransacao).toHaveTextContent('- R$ 50');
  });
});
